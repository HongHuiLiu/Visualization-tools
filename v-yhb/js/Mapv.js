...
geoTransform.wgs2bd(116.32715863448607,39.990912172420714)
        for (var j = 0; j < data.length; j++) {
            ...
            if (data[j].geo) {
                ...
                if (this.getCoordType() === 'bd09ll') {
                    ...
                    for (var i = 0; i < data[j].geo.length; i++) {
                        var bd=geoTransform.wgs2bd(data[j].geo[i][0],data[j].geo[i][1]);
                        var pixel = map.pointToPixel(new BMap.Point(bd[0], bd[1]));
                        tmp.push([pixel.x, pixel.y, parseFloat(data[j].geo[i][2])]);
                    }
                }
                data[j].pgeo = tmp;
